...
void c_trap_handler(unsigned int cause) { 
    print_string("TRAP! Cause: "); print_int(cause); while(1);
}

void test_cordic(const char* label, int32_t angle_q28) {
    print_string("CORDIC ["); print_string(label); print_string("]: ");
    ...
    *CORDIC_ANGLE = angle_q28;
    ...
    print_string("Sin="); print_hex(s);
    print_string(" Cos="); print_hex(c);
    print_string("\n");
}
...
void test_systolic_identity() {
    ...
    for (int i = 0; i < 16; i++) SYS_WEIGHT_BASE[i] = 0;
    SYS_WEIGHT_BASE[0] = 1; SYS_WEIGHT_BASE[5] = 1; 
    SYS_WEIGHT_BASE[10] = 1; SYS_WEIGHT_BASE[15] = 1;

    // 2. Load Vector [10, 20, 30, 40]
    SYS_ACT_BASE[0] = 10; SYS_ACT_BASE[1] = 20; 
    SYS_ACT_BASE[2] = 30; SYS_ACT_BASE[3] = 40;

    // 3. Pulse steps (Latency = 7)
    // FIX 2: Do NOT clear inputs after the first cycle.
    // The skewed wavefront pipeline needs the input vector held steady
    // for the full 7 cycles so each row's delay buffer captures the data.
    for (int i = 0; i < 7; i++) {
        *SYS_STEP = 1;
    }
    ...
    // 4. Print Results
    print_string("Out=[");
    print_int(SYS_OUT_BASE[0]); print_string(",");
    print_int(SYS_OUT_BASE[1]); print_string(",");
    print_int(SYS_OUT_BASE[2]); print_string(",");
    print_int(SYS_OUT_BASE[3]); print_string("]\n");
}
...
int main() {
    ...
    for (int i = 0; i < 16; i++) SYS_WEIGHT_BASE[i] = (i%5==0) ? 2 : 0;
    SYS_ACT_BASE[0] = 5; SYS_ACT_BASE[1] = 10; 
    SYS_ACT_BASE[2] = 15; SYS_ACT_BASE[3] = 20;
    // FIX 2: Keep inputs steady for the full wavefront latency
    for (int i = 0; i < 7; i++) {
        *SYS_STEP = 1;
    }
    print_string("Out=[");
